...
const TableEditDeleteRestoreActions = props => {
    ...
    const { record, condition, equalsTo, handleDelete, handleEdit, handleRestore } = props;

    return (
        <ButtonGroup>
            {condition.toString() == equalsTo ?
                <React.Fragment>
                    <Button type="primary" icon="edit" onClick={handleEdit} />
                    <Popconfirm placement='topLeft' okText={<FormattedMessage id='popconfirm.button.ok' />} cancelText={<FormattedMessage id='popconfirm.button.cancel' />} title={<FormattedMessage id='popconfirm.entity.delete' />} onConfirm={handleDelete}>
